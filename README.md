...
Optional:
- `GEMINI_MODEL` (optional model override, e.g. `gemini-1.5-flash-latest`)
...
- **Gemini model NOT_FOUND** (example: `models/gemini-1.5-flash is not found`):
  - If you set `GEMINI_MODEL`, use an available value such as `gemini-2.0-flash` or `gemini-2.0-flash-exp` (you can also include/exclude the `models/` prefix; both are handled).
  - If unsure, remove `GEMINI_MODEL` entirely and let auto-discovery choose from `ListModels`.
  - Restart `npm run dev` after env changes.
  - The app tries discovered models first (`v1beta` + `v1`), then falls back to modern known IDs (`gemini-2.5`, `gemini-flash-latest`, `gemini-2.0` variants).
- **`npm run lint` shows `Invalid project directory .../VoiceAgent/lint`**:
  - This happens with newer Next CLI behavior in some setups.
  - This repo uses `next lint --dir .` so lint is pinned to the current project.
  - Pull latest changes and re-run `npm run lint`.
- **`git pull` fails due to local `.DS_Store`, `.next`, or `node_modules` changes**:
  - These should be ignored; this repo now includes a `.gitignore` for them.
  - Clean local noise and retry:
    - `git reset --hard`
    - `git clean -fd`
    - `git pull`
