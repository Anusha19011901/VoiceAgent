...
- **Gemini model NOT_FOUND** (example: `models/gemini-1.5-flash is not found`):
  - If you set `GEMINI_MODEL`, make sure it exists for your API key/project; if unsure, remove `GEMINI_MODEL` to let auto-discovery choose.
  - Restart `npm run dev` after env changes.
  - The app fetches available models via Gemini `ListModels` and tries them (across `v1beta` + `v1`) before falling back to static defaults.
