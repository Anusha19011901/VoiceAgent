...
- **Gemini model NOT_FOUND** (example: `models/gemini-1.5-flash is not found`):
  - Set `GEMINI_MODEL` in `.env.local` to a model your account has (for example `gemini-2.0-flash`), then restart `npm run dev`.
  - The app also fetches available models via Gemini `ListModels` and tries multiple fallbacks automatically, but explicit override is safest when account/model access differs.
